...
function onJsonKitsu(json) {
    ...
    if (manga) {
        ...
        a.addEventListener('click', function(){inserisciRicerca(titolo);});
...
// Funzioni mostra risultati

function onResponseMostraRisultati(response){
    return response.json();
}

function onJsonMostraRisultati(json) {
...
function onErrorMostraRisultati(error){
    console.error(error);
}

function impostaRisultati(){
    fetch('getRisultati.php').then(onResponseMostraRisultati).then(onJsonMostraRisultati).catch(onErrorMostraRisultati);
}
